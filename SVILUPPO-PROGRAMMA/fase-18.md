...
È stata strutturata l'entità "esemplare" per mappare ogni singola copia fisica di un libro caricata da un utente (stato d'usura, note, disponibilità, collegamenti bibliografici, tassonomia a due livelli), unitamente alle funzioni CRUD dedicate.  
...
1. **Modellazione Concettuale e Relazionale (IFLA LRM / FRBR) & Tassonomia Gerarchica Ibrida a Due Livelli:**
   In aderenza ai modelli internazionali di catalogazione e archiviazione bibliografica *IFLA LRM* (*Library Reference Model*) e *FRBR* (*Functional Requirements for Bibliographic Records*), l'entità **Esemplare** (*Item*) è stata formalizzata come istanza fisica, tangibile e localizzata di un'opera intellettuale.
   È stato inoltre adottato il modello accademico di **Tassonomia Gerarchica Ibrida a Due Livelli** basato sugli standard editoriali internazionali *Thema* e *BISAC*:
   - **Livello 1 (Macro-Aree Disciplinari Controllate):** 10 categorie standardizzate e mutuamente esclusive (`categorie`), ciascuna provvista di icona Bootstrap Icons, codice colore tematico esadecimale e array di sottogeneri predefiniti (`sottogeneri_predefiniti TEXT[]`);
   - **Livello 2 (Sottogenere / Tag Tematico Flessibile):** colonna indicizzata `sottogenere VARCHAR(100)` sull'entità `esemplari`, che unisce la facilità di completamento guidato (datalist e chip interattivi Thema/BISAC) alla flessibilità di tag liberi per l'utente;
...
2. **Aggiornamento Schema DDL e Migrazione Database (`INFO-DATABASE/schema.sql`):**
   - Aggiunta delle colonne `editore VARCHAR(150)`, `lingua VARCHAR(50) DEFAULT 'Italiano'`, `note TEXT`, e `sottogenere VARCHAR(100)` su `esemplari`;
   - Aggiunta delle colonne `icona VARCHAR(50)`, `colore_hex VARCHAR(20)`, `sottogeneri_predefiniti TEXT[]` su `categorie`;
   ...
   - Creazione di indici B-Tree dedicati per l'ottimizzazione delle ricerche e dei join relazionali (`idx_esemplari_utente`, `idx_esemplari_categoria`, `idx_esemplari_sottogenere`, `idx_esemplari_disponibilita`, `idx_esemplari_titolo`).
3. **Generazione Dataset di Collaudo (`DATI-MOCK/esemplari_mock.md` e `server/seed_esemplari.js`):**
   - Popolamento iniziale nel database PostgreSQL di 10 volumi mock distribuiti tra i 4 utenti attivi e le 10 macro-categorie disciplinari, corredati da specifici sottogeneri di Livello 2;
   - Validazione di scenari di prossimità locale ($1.5\text{ km}$ e $6.4\text{ km}$), scambi metropolitani e isolamento geografico interurbano.
4. **Livello di Servizio Applicativo (`server/src/services/esemplariService.js`):**
   ...
   - Supporto ai filtri congiunti per macro-categoria (`categoria_id`) e sottogenere (`sottogenere`);
...
5. **Controller RESTful e Routing Express (`esemplariController.js`, `esemplariRoutes.js`, `categorieRoutes.js`):**
   ...
   - Esposizione del parametro di query `sottogenere` sia nel catalogo pubblico che nella libreria personale.
6. **Interfaccia Grafica e Vista Web (`hermae-frontend/libreria.html`):**
   ...
   - Barra di ricerca full-text singola a tutta larghezza e 4 selettori di filtro compattati su una singola riga responsive (*Macro-Area*, *Sottogenere*, *Disponibilità*, *Usura*);
   - Tre modalità di visualizzazione commutabili dinamicamente:
     - **Tabella:** Griglia accessibile con badge macro-area, pill di sottogenere, miniatura copertina tematica e azioni rapide;
     - **Card:** Vista a schede moderne con mockup copertina rigida, etichetta doppia macro/sottogenere e note bibliografiche;
     - **Book-Stile:** Scaffale virtuale 3D con libri fisici personalizzati per categoria, tag sottogenere sulla targhetta ex-libris e nell'overlay, costola goffrata, nastro segnalibro in tessuto e asse in legno massiccio;
   - Finestre modali accessibili per censimento/modifica con completamento guidato Thema/BISAC (`datalist`) e chip interattivi rapidi;
...
7. **Suite di Test Automatizzata (`server/test_fase18.js`):**
   - Esecuzione di 36 asserzioni di collaudo (validazioni 400, protezione 401, titolarità 403, CRUD 200/201, rimozione 404, tassonomia a 2 livelli con filtri combinati, regressione Fasi 16 e 17 al 100%).
...
- **Indici B-Tree Geografici e Relazionali:**
  ...
  - `idx_esemplari_sottogenere` su `sottogenere`: accelera il filtraggio su tag e sottogeneri Thema/BISAC;
...
| :---: | :--- | :---: | :--- | :---: |
| `GET` | `/api/esemplari/mie` | **Sì (Bearer)** | Elenco volumi posseduti con filtri opzionali (`categoria_id`, `sottogenere`, `stato`) | `200`, `401` |
| `POST` | `/api/esemplari` | **Sì (Bearer)** | Registrazione esemplare (L1 macro-area + L2 sottogenere + geoloc ereditata) | `201`, `400`, `401` |
| `GET` | `/api/esemplari/:id` | No | Dettaglio singolo esemplare con dati aggregati di categoria, sottogenere e proprietario | `200`, `404` |
...
| `GET` | `/api/esemplari` | No | Ricerca catalogo con filtri testuali (`search`), `categoria_id` e `sottogenere` | `200` |
| `GET` | `/api/categorie` | No | Elenco 10 macro-categorie con metadati (icona, colore HEX, sottogeneri Thema/BISAC) | `200` |
...
[GRUPPO 1] Tassonomia Categorie Disciplinari (Livello 1 e 2)
  ✓ PASS: GET /api/categorie restituisce 200 OK
  ✓ PASS: Macro-categorie presenti = 10 (standard Thema/BISAC)
  ✓ PASS: Categoria "narrativa-romanzi" identificata correttamente
  ✓ PASS: Metadati grafici icona e colore presenti
  ✓ PASS: Array sottogeneri predefiniti Thema/BISAC valorizzato
...
[GRUPPO 4] Creazione Nuovo Esemplare & Validazioni
  ...
  ✓ PASS: Sottogenere (Livello 2) memorizzato correttamente
...
[GRUPPO 5] Recupero Dettaglio Esemplare per ID
  ...
  ✓ PASS: Dettaglio include sottogenere L2
...
[GRUPPO 7] Aggiornamento Metadati da Proprietario (updateBook)
  ...
  ✓ PASS: Sottogenere aggiornato con successo a "Narrativa Filosofica"
...
[GRUPPO 10] Filtri Tassonomia Ibrida a Due Livelli (searchBooks)
  ...
  ✓ PASS: Tutti i risultati appartengono alla macro-categoria richiesta
  ✓ PASS: GET /api/esemplari?sottogenere=... restituisce 200 OK
  ✓ PASS: Trovati 2 volumi nel sottogenere "Ingegneria del Software"
  ✓ PASS: Tutti i volumi filtrati matchano esattamente il sottogenere

================================================================
 RIEPILOGO TEST FASE 18: 36 PASSATI, 0 FALLITI (100% SUCCESS RATE)
================================================================
```
